	...
	, _freewheeling (false)
{
	...
	vlc_cond_init (&_ready);
	vlc_clone (&_thread, worker_func, this, 0);
	while (!_run) {
#ifdef __APPLE__
		sched_yield ();
#elif defined _WIN32
...
Lv2Worker::~Lv2Worker ()
{
	...
	_run = false;
	vlc_cond_signal (&_ready);
	vlc_mutex_unlock (&_lock);

	vlc_join (_thread, NULL);
	vlc_mutex_destroy (&_lock);
	vlc_cond_destroy (&_ready);
}
